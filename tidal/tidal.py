...
class VALIDOBJECT():
    ...
    def __init__(self,resfile,**kwargs):
        ...
        self.funcnames = ['scanSELAFIN','SELAFIN','checkVar','getMesh','readOBS','nearestNodes','gettelArray','getobsArray','calcRMSE','plotting']
        self.timing = []
        ...
        if os.path.isfile(fullpath):
            ...
            t2 = datetime.now()
            self.timing.append((t2-t1).total_seconds())
            #timerend            
            newscan.printHeader()
            #timer
            t1 = datetime.now()
    ...
    def plotAll(self):
        ...
        for obsfile in obsfiles:
            ...
            if len(nodes)>0:
                ...
                #obs, = plt.plot(obsarray,'kx',ms=5.0,mew=2.0,label='Observations')
                x_pos = np.arange(len(obsarray))
                obs, = plt.plot(x_pos-0.25,obsarray,'r-.',linewidth=3.0,label='Observations') 
                ...
                self.timing.append((t2-t1).total_seconds())
                #timerend
                y_pos = np.arange(len(self.funcnames))
                plt.barh(y_pos,self.timing[0:len(self.funcnames)],align='center')
                plt.yticks(y_pos,self.funcnames)
                plt.show()
                plt.close()
    ...
    def getRmsepks(self,obsarray,telarray):
        ...
        for i in range(len(telarray[1,:])):                               
            ...
            meandiffsq = np.mean(diffsq)
            rmsearray[i] = (meandiffsq)**0.5
